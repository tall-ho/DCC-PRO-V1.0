...
const INITIAL_LOGS = [
    { id: '2P-26/001', date: '2026-03-20', company: 'Supplier A', auditor: 'External Body', findType: 'Minor', status: 'Approved' },
    { id: '2P-26/002', date: '2026-04-05', company: 'Co-Packer X', auditor: 'T All Team', findType: 'None', status: 'Approved' },
    { id: '2P-26/003', date: '2026-05-12', company: 'Logistics Partner', auditor: 'T All Team', findType: 'Major', status: 'Under Review' },
    { id: '2P-26/004', date: '2026-06-01', company: 'Supplier B', auditor: 'External Body', findType: 'Minor', status: 'Draft' },
];
...
const PIE_DATA = [
  { name: 'Approved', value: 2, color: THEME.success },
  { name: 'Under Review', value: 1, color: THEME.gold },
  { name: 'Draft', value: 1, color: THEME.dustyBlue },
];

const StatusBadge = ({ status }: { status: string }) => {
  const styles: any = {
    'Approved': { bg: '#657f4d15', text: THEME.success, border: '#657f4d30', icon: Icons.CheckCircle2 },
    'Under Review': { bg: '#b58c4f15', text: THEME.gold, border: '#b58c4f30', icon: Icons.Activity },
    ...
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-[200] w-full md:w-[500px] bg-white shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col border-l-2 border-[#b7a159] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-between items-center p-5 px-6 border-b-2 border-[#b7a159] bg-[#212c46] text-white shrink-0">
          <div>
            <h3 className="font-black flex items-center gap-3 uppercase tracking-widest text-lg"><Icons.Building size={22} className="text-[#b7a159]"/> 2ND PARTY AUDIT GUIDE</h3>
            <p className="text-[12px] font-bold text-[#d7d7d7] uppercase tracking-widest mt-1.5">Supplier & Partner Audit Management</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 text-[#414757] text-[12px] leading-relaxed custom-scrollbar bg-white">
          <section className="animate-fadeIn">
            ...
            <p className="text-[12px] mb-3">หน้าบันทึกการตรวจประเมินบุคคลที่สอง (ผู้จัดจำหน่าย / ผู้รับจ้างช่วง):</p>
            <ul className="list-disc pl-5 mt-2 space-y-2 text-[12px]">
                <li><strong className="text-[#4d87a8]">Dashboard:</strong> ติดตามและวิเคราะห์ภาพรวมการตรวจผู้ผลิต</li>
                <li><strong className="text-[#212c46]">Registry:</strong> ค้นหาและดูรายละเอียดของการตรวจประเมินแต่ละครั้ง</li>
            </ul>
          </section>
        </div>
...
export default function SecondPartyReportLog() {
        ...
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-[#212c46]/60 backdrop-blur-sm animate-fadeIn">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-[#eaeaec] flex flex-col">
                <div className="px-6 py-4 border-b border-[#eaeaec] bg-[#f8f9fa] flex justify-between items-center">
                    <h3 className="font-black text-[#212c46] uppercase tracking-widest text-[14px] flex items-center gap-2">
                        {isFilterOpen ? <><Icons.Filter size={18} className="text-[#3f809e]"/> Filter Records</> :
                         isNewReportOpen ? <><Icons.Briefcase size={18} className="text-[#success]"/> Create New Audit</> :
                         viewReportId ? <><Icons.Eye size={18} className="text-[#b58c4f]"/> View Partner Audit</> :
                         <><Icons.Edit3 size={18} className="text-[#a94228]"/> Edit Partner Audit</>}
                    </h3>
                ...
                <div className="p-8 text-[#414757] text-[12px] font-bold">
                    {isFilterOpen && <p className="text-center text-[#7a8b95] py-10">Filter Options for 2nd Party Log</p>}
                    {isNewReportOpen && <p className="text-center text-[#7a8b95] py-10">Input standard form for New 2nd Party Audit</p>}
                    {viewReportId && <p className="text-center text-[#7a8b95] py-10">Detailed view for Reference: <span className="text-[#932c2e] font-black">{viewReportId}</span></p>}
                    {editReportId && <p className="text-center text-[#7a8b95] py-10">Edit view for Reference: <span className="text-[#932c2e] font-black">{editReportId}</span></p>}
                </div>
                <div className="px-6 py-4 bg-[#f8f9fa] border-t border-[#eaeaec] flex justify-end gap-3">
                    ...
                    {!viewReportId && (
                      <button onClick={() => { 
                          ...
                          showToast(isFilterOpen ? "Filters Applied" : isNewReportOpen ? "Audit Created Successfully" : "Changes Saved"); 
          ...
          <div className="flex items-center gap-5">
              <div className="relative flex items-center justify-center group cursor-default shrink-0">
                  ...
                  <div className="relative z-10 p-1.5 border border-[#3f809e]/40 rounded-xl bg-white/50 backdrop-blur-sm shadow-sm">
                      <Icons.Building size={28} strokeWidth={2.5} className="text-[#3f809e]" />
                  </div>
              </div>
              <div>
                  <h3 className="font-black text-[#212c46] uppercase tracking-tighter leading-none" style={{ fontSize: '24px' }}>
                      2ND PARTY <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#3f809e] to-[#b58c4f]">AUDIT LOG</span>
                  </h3>
                  <p className="text-[11px] font-bold text-[#4d5a44] uppercase tracking-[0.2em] mt-0.5 opacity-80 leading-none">
                      SUPPLIER & OUTSOURCED SERVICE AUDIT PERFORMANCE
                  </p>
              </div>
          </div>
        ...
        {activeTab === 'dashboard' ? (
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                <KpiCard label="Total Suppliers" value={INITIAL_LOGS.length} icon="Building" colorAccent={THEME.primaryLight} colorValue={THEME.primary} desc="Audited YTD" />
                <KpiCard label="Under Review" value={INITIAL_LOGS.filter(l => l.status === 'Under Review').length} icon="Clock" colorAccent={THEME.gold} colorValue={THEME.primary} desc="Awaiting Actions" />
                <KpiCard label="Major NCs" value={1} icon="AlertTriangle" colorAccent={THEME.danger} colorValue={THEME.danger} desc="Critical Findings" />
                <KpiCard label="Approved" value={INITIAL_LOGS.filter(l => l.status === 'Approved').length} icon="CheckCircle" colorAccent={THEME.success} colorValue={THEME.primary} desc="Partners Cleared" />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                <div className="lg:col-span-8 bg-white rounded-3xl shadow-lg border border-[#eaeaec] p-6">
                    <h3 className="text-[14px] font-black text-[#212c46] uppercase tracking-widest flex items-center gap-2 border-b border-[#eaeaec] pb-4 mb-6"><Icons.BarChart2 size={18} className="text-[#b7a159]" /> AUDIT FREQUENCY BY MONTH</h3>
                    <div className="h-[300px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={BAR_DATA} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                                ...
                                <Bar dataKey="count" fill={THEME.skyBlue} radius={[4, 4, 0, 0]} barSize={40} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                ...
                <div className="lg:col-span-4 bg-white rounded-3xl shadow-lg border border-[#eaeaec] p-6">
                    <h3 className="text-[14px] font-black text-[#212c46] uppercase tracking-widest flex items-center gap-2 border-b border-[#eaeaec] pb-4 mb-6"><Icons.PieChart size={18} className="text-[#b7a159]" /> STATUS BREAKDOWN</h3>
              ...
              <div className="px-8 py-4 border-b border-[#eaeaec] bg-[#f8f9fa] flex flex-col md:flex-row justify-between items-center gap-4 shrink-0">
                  <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => showToast('Printing Archive...')} className="bg-white border border-[#eaeaec] text-[#212c46] px-5 py-2 rounded-xl font-black uppercase text-[12px] shadow-sm hover:border-[#b7a159] transition-all flex items-center gap-2">
                        <Icons.Printer size={14} /> PRINT ARCHIVE
                      </button>
                  </div>
                  <div className="flex items-center gap-3 w-full md:w-auto">
                      <div className="relative flex-1 md:w-80">
                          <Icons.Search size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-[#7a8b95]" />
                          <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search Reference, Company..." className="w-full pl-12 pr-6 py-2.5 text-[12px] border border-[#eaeaec] rounded-xl font-bold outline-none focus:border-[#b7a159] bg-white shadow-sm text-[#212c46]" />
                      </div>
                  ...
                  <table className="w-full text-left font-sans border-collapse min-w-[1000px]">
                      <thead className="bg-[#1d2636] text-white sticky top-0 z-10">
                          <tr className="border-b-[3px] border-[#b7a159]">
                              <th className="py-4 px-6 font-black uppercase tracking-widest text-[11px] whitespace-nowrap">Ref No</th>
                              <th className="py-4 px-6 font-black uppercase tracking-widest text-[11px] whitespace-nowrap">Date</th>
                              <th className="py-4 px-6 font-black uppercase tracking-widest text-[11px] whitespace-nowrap">Target Company</th>
                              <th className="py-4 px-6 font-black uppercase tracking-widest text-[11px] whitespace-nowrap">Auditor Body</th>
                              ...
                              <tr key={i} className="hover:bg-[#f8f9fa] transition-colors group">
                                  <td className="py-3 px-6 font-black text-[#932c2e] uppercase tracking-tight text-[12px]">
                                      <div className="flex items-center gap-2">
                                          <Icons.Briefcase size={14} className="text-[#a54f6b]" />
                                          {log.id}
                                      </div>
                                  </td>
                                  <td className="py-3 px-6 font-bold text-[#4d87a8] uppercase text-[12px]">{log.date}</td>
                                  <td className="py-3 px-6 font-black text-[#212c46] text-[12px]">{log.company}</td>
