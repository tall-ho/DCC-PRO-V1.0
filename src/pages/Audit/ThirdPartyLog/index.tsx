...
const INITIAL_LOGS = [
    { id: '3P-26/001', date: '2026-05-12', body: 'SGS Thailand', standard: 'ISO 9001:2015', findType: 'Minor', status: 'Certified', validUntil: '2029-05-12' },
    { id: '3P-26/002', date: '2026-03-10', body: 'TUV Nord', standard: 'ISO 14001:2015', findType: 'None', status: 'Certified', validUntil: '2029-03-10' },
    { id: '3P-26/003', date: '2026-01-15', body: 'BSI Group', standard: 'GHPs & HACCP', findType: 'Major', status: 'In Progress', validUntil: '-' },
    { id: '3P-25/008', date: '2025-08-20', body: 'SGS Thailand', standard: 'FSSC 22000', findType: 'Minor', status: 'Expired', validUntil: '2026-08-20' },
];
...
const PIE_DATA = [
  { name: 'Certified', value: 2, color: THEME.success },
  { name: 'In Progress', value: 1, color: THEME.gold },
  { name: 'Expired', value: 1, color: THEME.danger },
];

const StatusBadge = ({ status }: { status: string }) => {
  const styles: any = {
    'Certified': { bg: '#657f4d15', text: THEME.success, border: '#657f4d30', icon: Icons.Award },
    'In Progress': { bg: '#b58c4f15', text: THEME.gold, border: '#b58c4f30', icon: Icons.Activity },
    'Expired': { bg: '#932c2e15', text: THEME.danger, border: '#932c2e30', icon: Icons.XCircle },
  };
  const config = styles[status] || styles['In Progress'];
  const Icon = config.icon;
  return (
    <span className="px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-1.5 border shadow-sm w-[120px]" style={{ backgroundColor: config.bg, color: config.text, borderColor: config.border }}>
    ...
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-[200] w-full md:w-[500px] bg-white shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col border-l-2 border-[#b7a159] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-between items-center p-5 px-6 border-b-2 border-[#b7a159] bg-[#212c46] text-white shrink-0">
          <div>
            <h3 className="font-black flex items-center gap-3 uppercase tracking-widest text-lg"><Icons.Award size={22} className="text-[#b7a159]"/> 3RD PARTY AUDIT GUIDE</h3>
            <p className="text-[12px] font-bold text-[#d7d7d7] uppercase tracking-widest mt-1.5">Certification & External Body Tracking</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 text-[#414757] text-[12px] leading-relaxed custom-scrollbar bg-white">
          <section className="animate-fadeIn">
            ...
            <p className="text-[12px] mb-3">หน้าบันทึกการตรวจประเมินบุคคลที่สาม (ระบบมาตรฐานและการรับรอง):</p>
            <ul className="list-disc pl-5 mt-2 space-y-2 text-[12px]">
                <li><strong className="text-[#4d87a8]">Dashboard:</strong> สรุปสถานะการรับรอง และการตรวจรับรองของ CB</li>
                <li><strong className="text-[#212c46]">Registry:</strong> บันทึก Certification Body, วันที่หมดอายุ (Valid Until) และ NCs</li>
            </ul>
          </section>
        </div>
...
export default function ThirdPartyReportLog() {
  ...
  const filteredLogs = useMemo(() => {
    return INITIAL_LOGS.filter(l => 
      ...
      l.standard.toLowerCase().includes(search.toLowerCase())
    );
  }, [search]);
        ...
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-[#212c46]/60 backdrop-blur-sm animate-fadeIn">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-[#eaeaec] flex flex-col">
                <div className="px-6 py-4 border-b border-[#eaeaec] bg-[#f8f9fa] flex justify-between items-center">
                    <h3 className="font-black text-[#212c46] uppercase tracking-widest text-[14px] flex items-center gap-2">
                        {isFilterOpen ? <><Icons.Filter size={18} className="text-[#3f809e]"/> Filter Records</> :
                         isNewReportOpen ? <><Icons.Award size={18} className="text-[#success]"/> Create New Audit Record</> :
                         viewReportId ? <><Icons.Eye size={18} className="text-[#b58c4f]"/> View Cert Details</> :
                         <><Icons.Edit3 size={18} className="text-[#a94228]"/> Edit Cert Details</>}
                    </h3>
                ...
                <div className="p-8 text-[#414757] text-[12px] font-bold">
                    {isFilterOpen && <p className="text-center text-[#7a8b95] py-10">Filter Options for 3rd Party Certs</p>}
                    {isNewReportOpen && <p className="text-center text-[#7a8b95] py-10">Input standard form for New Certification Audit</p>}
                    {viewReportId && <p className="text-center text-[#7a8b95] py-10">Detailed view for Certificate Ref: <span className="text-[#932c2e] font-black">{viewReportId}</span></p>}
                    {editReportId && <p className="text-center text-[#7a8b95] py-10">Edit view for Certificate Ref: <span className="text-[#932c2e] font-black">{editReportId}</span></p>}
                </div>
                <div className="px-6 py-4 bg-[#f8f9fa] border-t border-[#eaeaec] flex justify-end gap-3">
                    ...
                    {!viewReportId && (
                      <button onClick={() => { 
                          ...
                          showToast(isFilterOpen ? "Filters Applied" : isNewReportOpen ? "Record Created Successfully" : "Changes Saved"); 
          ...
          <div className="flex items-center gap-5">
              <div className="relative flex items-center justify-center group cursor-default shrink-0">
                  ...
                  <div className="relative z-10 p-1.5 border border-[#a94228]/40 rounded-xl bg-white/50 backdrop-blur-sm shadow-sm">
                      <Icons.Award size={28} strokeWidth={2.5} className="text-[#a94228]" />
                  </div>
              </div>
              <div>
                  <h3 className="font-black text-[#212c46] uppercase tracking-tighter leading-none" style={{ fontSize: '24px' }}>
                      3RD PARTY <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#a94228] to-[#b58c4f]">AUDIT LOG</span>
                  </h3>
                  <p className="text-[11px] font-bold text-[#4d5a44] uppercase tracking-[0.2em] mt-0.5 opacity-80 leading-none">
                      EXTERNAL CERTIFICATION BODIES & STANDARDS
                  </p>
              </div>
          </div>
        ...
        {activeTab === 'dashboard' ? (
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                <KpiCard label="Active Certs" value={INITIAL_LOGS.filter(l => l.status === 'Certified').length} icon="Award" colorAccent={THEME.success} colorValue={THEME.primary} desc="Valid Standards" />
                <KpiCard label="In Progress" value={INITIAL_LOGS.filter(l => l.status === 'In Progress').length} icon="Clock" colorAccent={THEME.gold} colorValue={THEME.primary} desc="Awaiting Results" />
                <KpiCard label="Upcoming Expiry" value={1} icon="AlertTriangle" colorAccent={THEME.danger} colorValue={THEME.danger} desc="Expires < 90 Days" />
                <KpiCard label="Audit Bodies" value={3} icon="Globe" colorAccent={THEME.primaryLight} colorValue={THEME.primary} desc="Total CBs" />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                <div className="lg:col-span-8 bg-white rounded-3xl shadow-lg border border-[#eaeaec] p-6">
                    <h3 className="text-[14px] font-black text-[#212c46] uppercase tracking-widest flex items-center gap-2 border-b border-[#eaeaec] pb-4 mb-6"><Icons.BarChart2 size={18} className="text-[#a94228]" /> AUDIT FREQUENCY BY MONTH</h3>
                ...
                <div className="lg:col-span-4 bg-white rounded-3xl shadow-lg border border-[#eaeaec] p-6">
                    <h3 className="text-[14px] font-black text-[#212c46] uppercase tracking-widest flex items-center gap-2 border-b border-[#eaeaec] pb-4 mb-6"><Icons.PieChart size={18} className="text-[#a94228]" /> STATUS BREAKDOWN</h3>
          ...
          <div className="bg-white/90 rounded-3xl shadow-lg border border-[#eaeaec] overflow-hidden flex flex-col animate-fadeIn min-h-[600px]">
              <div className="px-8 py-4 border-b border-[#eaeaec] bg-[#f8f9fa] flex flex-col md:flex-row justify-between items-center gap-4 shrink-0">
                  <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => showToast('Printing Archive...')} className="bg-white border border-[#eaeaec] text-[#212c46] px-5 py-2 rounded-xl font-black uppercase text-[12px] shadow-sm hover:border-[#b7a159] transition-all flex items-center gap-2">
                        <Icons.Printer size={14} /> PRINT RECORDS
                      </button>
                  </div>
                  <div className="flex items-center gap-3 w-full md:w-auto">
                      <div className="relative flex-1 md:w-80">
                          <Icons.Search size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-[#7a8b95]" />
                          <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search Ref No, CB, Standard..." className="w-full pl-12 pr-6 py-2.5 text-[12px] border border-[#eaeaec] rounded-xl font-bold outline-none focus:border-[#b7a159] bg-white shadow-sm text-[#212c46]" />
                      </div>
              ...
              <div className="overflow-auto custom-scrollbar flex-1">
                  <table className="w-full text-left font-sans border-collapse min-w-[1000px]">
                      <thead className="bg-[#1d2636] text-white sticky top-0 z-10">
                          <tr className="border-b-[3px] border-[#a94228]">
                              <th className="py-4 px-6 font-black uppercase tracking-widest text-[11px] whitespace-nowrap">Cert Ref No</th>
                              <th className="py-4 px-6 font-black uppercase tracking-widest text-[11px] whitespace-nowrap">Audit Date</th>
                              <th className="py-4 px-6 font-black uppercase tracking-widest text-[11px] whitespace-nowrap">Body Name (CB)</th>
                              <th className="py-4 px-6 font-black uppercase tracking-widest text-[11px] whitespace-nowrap">Standard</th>
                              <th className="py-4 px-6 font-black uppercase tracking-widest text-[11px] text-center">Finding</th>
                              <th className="py-4 px-6 font-black uppercase tracking-widest text-[11px] text-center">Status</th>
                              <th className="py-4 px-6 font-black uppercase tracking-widest text-[11px] text-center">Valid Until</th>
                              ...
                              <tr key={i} className="hover:bg-[#f8f9fa] transition-colors group">
                                  <td className="py-3 px-6 font-black text-[#932c2e] uppercase tracking-tight text-[12px]">
                                      <div className="flex items-center gap-2">
                                          <Icons.Globe size={14} className="text-[#a54f6b]" />
                                          {log.id}
                                      </div>
                                  </td>
                                  <td className="py-3 px-6 font-bold text-[#4d87a8] uppercase text-[12px]">{log.date}</td>
                                  <td className="py-3 px-6 font-black text-[#212c46] text-[12px]">{log.body}</td>
                                  <td className="py-3 px-6 font-bold text-[#7a8b95] text-[12px] uppercase">{log.standard}</td>
                                  <td className="py-3 px-6 text-center">
                                      <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border shadow-sm ${log.findType === 'None' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-600 border-red-200'}`}>
                                          {log.findType}
                                      </span>
                                  </td>
                                  ...
                                  <td className="py-3 px-6 text-center text-[12px] font-mono text-[#7a8b95] font-bold">
                                      {log.validUntil}
                                  </td>
                                  <td className="py-3 px-6 text-center">
